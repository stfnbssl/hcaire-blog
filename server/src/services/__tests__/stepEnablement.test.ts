// Test unit per evaluateStepEnablement (D4 §9 + D7 v3.0).
...
test('enablement: dipendenza required completato → enabled', () => {
  const cfg = step('f3_step_3', {
    inputs_pipeline: [
      { step: 'f3_step_1', role: 'nodo-funzione', required: true },
      { step: 'f3_step_2', role: 'micro-dispositivo', required: true },
    ],
  });
  const ctx = {
    step_states: {
...
test('enablement: dipendenza required mancante → blocked, ragione dependency', () => {
  const cfg = step('f3_step_2', {
    inputs_pipeline: [{ step: 'f3_step_1', role: 'nodo-funzione', required: true }],
  });
  const result = evaluateStepEnablement({}, cfg, NO_INPUTS);
...
test('enablement: requires_verifica:true accetta solo verificato/saltato', () => {
  const cfg = step('f3_step_2', {
    inputs_pipeline: [
      { step: 'f3_step_1', role: 'nodo-funzione', required: true, requires_verifica: true },
    ],
  });
  // status completato NON basta perché serve verificato
  const ctxCompletato = { step_states: { f3_step_1: { status: 'completato' as const } } };
  ...
  const ctxVerificato = { step_states: { f3_step_1: { status: 'verificato' as const } } };
...
test('enablement: status saltato accettato come dipendenza soddisfatta', () => {
  // Caso reale v3.0: f3_step_5 (audit metodologico) è skippabile; uno step che
  // dipende da step terminale accetta lo stato saltato come equivalente.
  const cfg = step('f3_step_X', {
    inputs_pipeline: [{ step: 'f3_step_5', role: 'audit', required: true }],
  });
  const ctx = { step_states: { f3_step_5: { status: 'saltato' as const } } };
  assert.equal(evaluateStepEnablement(ctx, cfg, NO_INPUTS).enabled, true);
});
...
test('enablement: input esterno obbligatorio mancante → blocked, ragione missing_input', () => {
  // Caso reale v3.0: f3_step_1 richiede `contesto_ambito` come input obbligatorio
  // (auto-popolato dal bridge ambiti, ma il check sul provided set è il medesimo).
  const cfg = step('f3_step_1', {
    inputs_pipeline: [{ step: 'f2_step_6', role: 'output-tipo-vuoto', required: true }],
    inputs_esterni: [
      { id: 'contesto_ambito', label: 'Contesto/ambito target', type: 'esterno_obbligatorio' },
    ],
  });
  const ctx = { step_states: { f2_step_6: { status: 'completato' as const } } };
...
test('enablement: input esterno facoltativo non blocca anche se mancante', () => {
  // Caso reale v3.0: f3_step_3 (Stress test) ha `casi_dominio` facoltativo.
  const cfg = step('f3_step_3', {
    inputs_esterni: [
      { id: 'casi_dominio', label: 'Casi reali del dominio', type: 'esterno_facoltativo' },
    ],
  });
  assert.equal(evaluateStepEnablement({}, cfg, NO_INPUTS).enabled, true);
});
...
test('enablement: dipendenze + input mancanti accumulati come blocking_reasons', () => {
  const cfg = step('f3_step_1', {
    inputs_pipeline: [
      { step: 'f2_step_6', role: 'output-tipo-vuoto', required: true },
    ],
    inputs_esterni: [
